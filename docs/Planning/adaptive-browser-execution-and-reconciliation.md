...
| Agent-facing browser contract                    | `apps/mcp/src/tools/browser.tools.ts`                                                               | **Generalized for ordinary interactions.** Codex supplies semantic outcomes; a deterministic MCP compiler translates them to private Runtime effects. Specialized semantic-transaction inputs remain lower-level pending Item 6 convergence. |
...
- specialized semantic-transaction tools still expose lower-level effect inputs;
...
**Status:** Complete for ordinary interactions and task-result commits.
...
Task-result preparation accepts the same semantic outcome vocabulary and stores its compiled effect basis in the durable Runtime plan. Commit does not accept a replacement outcome list: MCP retrieves the authorized plan and supplies its exact stored basis to Runtime, whose existing plan identity, material digest, and action fingerprint checks remain authoritative. Specialized semantic-transaction tools retain their low-level effect inputs for the Item 6 convergence review.
