...
| Accepted user conversation item | The single durable local user-authored conversation item created when Rove accepts input; immediately renderable in its final transcript position and owner of stable operation/client identity and delivery state. | Local; later provider materialization only corroborates or updates this same item. |
...
| Conversation entry    | An accepted user conversation item from the moment of acceptance, assistant output, or meaningful displayed work/result item. | Local; raw model internals are not a product record.                |
...
A task's organization state is active or archived; explicit deletion has separate cleanup rules. An accepted user conversation item is the durable user-authored Task conversation truth from acceptance onward, before provider dispatch or materialization. It is not an accepted-instruction record that later converts into a separate transcript entry. Definite non-submission or uncertain delivery changes the delivery and presentation state of that same item. Exact later App Server `userMessage` materialization correlates with and updates that item and never appends a second copy for the same operation/client identity. An execution turn is the bounded agent attempt that may deliver the item to the provider. A queued follow-up remains a durable pending Task instruction, not provider input or conversation truth; at the committed execution boundary, promotion atomically turns that exact queued instruction into the single accepted user conversation item. Stopping does not discard or execute an unpromoted follow-up.
