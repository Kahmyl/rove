...
Selecting an Output freezes its exact immutable Result revision and digest in a local selection record. A later draft edit may change the current revision without changing the selected one. The durable task event and outbox command record the exact task, Result, selected revision, content digest, lifecycle state, working context, and applicable host policy. Selection is consumed only after that exact snapshot is accepted; consumption compares the frozen revision and digest rather than whichever revision is current then. Switching task views keeps unsent follow-up text task-keyed, and selection cannot resolve a Result from another task.
