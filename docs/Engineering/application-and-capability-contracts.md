...
Archive is accepted only when no Codex turn is actively executing; it does not implicitly Stop. Pending resource cleanup does not block local Archive. Restore succeeds from the same local preference while Codex is unavailable. Provider thread unarchive or replacement is deferred until a later model-assisted message actually needs provider execution.
