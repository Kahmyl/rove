...
A consequential interaction crosses one mutation boundary under a stable consequence identity. Immediate post-action evidence may settle it as applied or not applied. If dispatch occurred but the requested outcome remains unverified, the mutation is fenced and the operation may enter bounded read-only reconciliation. Reconciliation remains attached to the same operation/receipt and may gather fresh evidence; it cannot repeat the effect, substitute another transport, alter authorized material, or create a new consequence under the guise of verification.
