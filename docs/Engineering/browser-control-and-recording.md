...
Codex reasons primarily in terms of the user's intended browser outcome and current grounded state. Ordinary `browser.interact` accepts semantic URL, visible-text, target, target-location, value, selection, file, page, and download outcomes; the MCP boundary deterministically translates them into Runtime verification effects. Consequential ordinary interactions require at least one usable outcome. A task-result commit reuses the exact effect basis stored in its authorized Runtime plan rather than accepting a second caller-supplied outcome basis. Runtime effects, target/scope predicates, correlation strategies, and evidence escalation remain internal safety mechanisms, so Codex does not need detailed knowledge of truncation or verifier implementation to choose a safe proof strategy. Specialized transaction tools still expose lower-level effect inputs pending convergence of their multi-step contract.
