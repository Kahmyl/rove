...
The desktop control path preserves the same ownership boundary. Take Over, Pause, Return Control, and browser presentation start from the exact Task acted on, resolve its durable Runtime association in the host, and address that session explicitly. Take/return operations additionally preserve the exact live handoff identity and generation; every ownership mutation carries the expected ownership generation into Runtime and fails before transition when it is stale. A completed, exact Rove `control.request_human` observation supplies the continuation instruction and policy; the pinned App Server MCP form and its schema-supported dynamic tool-item form share one normalizer before Runtime corroborates the returned handoff identity. An automatic Runtime transition to `awaiting_human` is not sufficient authority on its own: the task-bound MCP server refuses `control.wait` until `control.request_human` has successfully returned that same active handoff identity and generation. A fresh Runtime observation that exactly corroborates a durable pending continuation repairs a missing or stale customer-facing handoff attention projection; disagreement leaves takeover unavailable and the inconsistent projection stale. The selected Task, physically focused page, or most recently updated Runtime session is presentation evidence only. Task-specific recording commands and playback likewise resolve their stored Task/session association and cannot fall through to another task's page group.
...
Codex reasons primarily in terms of the user's intended browser outcome and current grounded state. Ordinary `browser.interact` and specialized transaction advance/verification tools accept the same semantic URL, visible-text, target, target-location, value, selection, file, page, and download outcome vocabulary; the MCP boundary deterministically translates it into private Runtime verification effects. Consequential ordinary interactions and transaction commits require at least one usable outcome, except for the exact selected-source trusted clipboard staging invariant during transaction prepare. A task-result commit reuses the exact effect basis stored in its authorized Runtime plan rather than accepting a second caller-supplied outcome basis. Runtime effects, target/scope predicates, correlation strategies, and evidence escalation remain internal safety mechanisms, so Codex does not need detailed knowledge of truncation or verifier implementation to choose a safe proof strategy.
...
If dispatch occurred but immediate evidence is insufficient, preserve the operation as dispatched/unverified and enter bounded read-only reconciliation. Ordinary consequential dispatch durably binds its exact effect predicates and predecessor proposition truth. A later `browser.reconcile_outcome` read can use a fresh same-task/workspace observation, canonical target evidence, and focused whole-page visible-text evidence to settle the same journal effect as applied or not applied without calling browser interaction. Pre-existing success and incomplete evidence remain unresolved. The original receipt and verification basis stay immutable, and only the exact consequence fence is released after optimistic terminal settlement is durable.

The implemented later-settlement family covers visible target presence/absence and whole-page visible text. Broader reconciliation may eventually fresh-inspect, wait for asynchronous convergence, inspect another target or semantic scope, search the application, navigate to an authoritative read surface, use screenshot/visual evidence, or correlate durable Runtime/browser evidence. It remains tied to the existing operation and consequence identity. It cannot repeat the mutation, change approved material, switch transport to evade the fence, or treat unrelated/pre-existing state as causal proof.
