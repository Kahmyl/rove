...
The selected `0.154.0-alpha.6.2` App Server currently blocks reliable local-command Stop semantics. Its generated protocol exposes opaque `commandExecution.processId` metadata but no exact command-cancel RPC. Upstream `unified_exec` is known to retain an in-flight process after `turn/interrupt` ([open issue](https://github.com/openai/codex/issues/42717)); live qualification also proved that launching this exact build with the supported `--disable unified_exec` override still allowed a turn-bound one-shot command to emit its delayed completion output after the interrupt RPC succeeded and the turn became `interrupted`. Rove must not guess OS process identity or issue broad kills from the opaque provider identifier. Reliable local-command Stop qualification remains blocked until a pinned App Server provides and qualifies exact turn-owned process cancellation, or another supported mode actually terminates the process in the live journey.
