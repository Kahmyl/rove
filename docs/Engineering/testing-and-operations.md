...
| Codex truth convergence | Deliberately lost listener/mapping/ingress terminals converge from exact bound history for turn, assistant, ordinary tool, user-message correlation, and request-human families without redispatch; duplicate, late, reconnect, restart, and multi-Task cases remain monotonic and isolated. |
...
| Conversation/task experience | Production projections prove immediate accepted-message display and exact materialization deduplication; anti-flicker startup; forced-open active work and terminal auto-compaction; semantic truthful activity; active-duration freeze/resume; durable Queue, restart, edit/remove and safe promotion; explicit Steer; Stop before provider turn, during a turn, and during waiting/checking with queued work retained; request-family attention; Agent handoff and Companion voluntary takeover; human ownership, Return to Rove and fresh page checking; internal recovery without false attention; distinct failure/uncertainty; coherent sidebar and multi-Task behavior; and main/follower semantic parity. |

The conversation/task experience must be exercised through the production TaskEngine, Product/Application projections, preload bridge, and rendered Electron surface for important journeys. Synthetic renderer fixtures alone are insufficient, especially fixtures that inject `finish`, invented lifecycle phases, or follower-only Runtime semantics unavailable from the production Task projection.

Functional assertions cover durable truth and permitted transitions. Rendered assertions cover customer copy, hierarchy, control presence and disabled/submitting states, stable hit targets, focus and keyboard behavior, reduced-motion-compatible transitions, active auto-follow and **Latest**, completion scroll anchoring, long titles/messages/attachments, many activities, normal and narrow layouts, and multiple simultaneously active or attention-bearing tasks. Screenshots supplement DOM/state assertions; they are not the only evidence.

Qualification includes confirmed failure, uncertain consequential effect, known model unavailability, and recovery unable to establish truth. It proves internal lifecycle diagnostics and browser observation/evidence counters do not appear on ordinary customer surfaces, and that a generic retry or attention treatment is not shown where unsafe or unnecessary.
...
The serialized production MCP `tools/list` contract is also a browser acceptance boundary. Its regression covers every browser tool, including ordinary interaction, task-result preparation, transaction advance, and transaction verification; it permits semantic outcomes, consequence identity, authoritative receipts, transaction state, destination context, and read-only reconciliation while rejecting private Runtime expected-effect property names and predicate kinds.
...
Codex adapter qualification deliberately drops terminal notifications after App Server history has materialized them, injects listener/ingress failure, repeats history, delivers stale started observations after repair, replaces the connection, and restarts from incomplete local state. At least one production-composition test must traverse realistic App Server history through ordered ingress and `TaskEngine` into the product snapshot. Cover unrelated reconstructible families rather than only handoff. For live-only server-request attention and provider archive membership, prove that history success cannot clear their blockers; only the exact matching live/provider authority can. Prove that a different App Server listener failure creates no false Task blocker and that expendable progress failure does not block terminal convergence. Diagnostics retain bounded recovery class, exact correlation, event family, Task/thread identity, attempt, outcome, and error category rather than raw payload.
