...
The application service owns product commands and task state. The Codex adapter owns transport, supported protocol conversion, engine-account association, event correlation, and bounded convergence of reconstructible exact App Server history into the local Task authority. Live notification delivery is a fast path; it is not itself durable truth. History repair re-enters the existing Task reducer and cannot dispatch new work or become a second lifecycle writer. Runtime/capability adapters own the truth they can actually observe. The renderer submits intent and renders projections, not a second task scheduler.
