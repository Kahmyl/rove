...
} from "./playwright-browser-engine.js";
import type {
  BrowserSession,
} from "./engine.js";
...
async function createBrowser() {
  const browser =
    await new PlaywrightBrowserEngine().start({
      headless: true,
      browser: "chromium",
      profile: {
        mode: "temporary",
...
describe("Milestone 9 browser activity foundation", () => {
  ...
  it("emits minimized interaction, scroll, selection, popup, and tab-switch activity", async () => {
    ...
    expect(switched.data).toHaveProperty(
      "url",
    );
