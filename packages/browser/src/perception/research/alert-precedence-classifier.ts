...
import type { Gate5Input } from "./classification-strategies.js";
import type {
  Gate6SemanticSurfaceV6,
  Gate6SurfaceFactsV6,
} from "./alert-precedence-semantics.js";

type Truth = true | false | "indeterminate";

export interface Gate6CandidateV6Input extends Gate5Input {
  surfaceFacts?: Gate6SurfaceFactsV6;
}
...
function surfaceEligibleForKnownBlocker(
  surface: Gate6SemanticSurfaceV6,
): boolean {
  return surface.kind === "blocking_dialog" || surface.kind === "primary";
}

export function gate6CandidateV6Strategy(): BenchmarkStrategy<Gate6CandidateV6Input> {
  return {
    name: "gate6-s4r6-evidence-precedence-alert-ownership",
    predict(input) {
