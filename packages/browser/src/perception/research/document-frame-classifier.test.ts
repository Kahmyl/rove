...
async function classify(body: string, title = "Test", httpStatus?: number) {
  ...
  try {
    ...
    const input: Gate6CandidateV7Input = {
      signals: await pageSignals(page, httpStatus),
      evidence: evidence.evidence,
      surfaceFacts: await collectGate6SurfaceFactsV7(page),
    };

    return gate6CandidateV7Strategy().predict(input, {
      id: "s4r7-metamorphic",
      tier: "A",
      description: "S4R7 metamorphic",
...
describe("document frame classifier", () => {
  ...
  describe("document-level verification frame ownership", () => {
    it("recognizes a presented verification iframe that is a sibling of main", async () => {
      const result = await classify(
        `
          <main>
            <h1>Workspace</h1>
      ...
      expect((await result).propositions?.humanVerificationPresented).toBe(
        true,
      );
    });

    it("keeps the same sibling verification iframe non-blocking when offscreen", async () => {
      const result = await classify(
        `
          <main>
            <h1>Workspace</h1>
    ...
    it("does not promote a verification iframe owned by supplementary content", async () => {
      const result = await classify(
        `
          <main>
            <h1>Integration docs</h1>
            <p>The primary document is available.</p>
