...
async function classify(body: string, title = "Test", httpStatus?: number) {
  ...
  try {
    ...
    const input: Gate6CandidateV4Input = {
      signals: await pageSignals(page, httpStatus),
      evidence: evidence.evidence,
      surfaceFacts: await collectGate6SurfaceFactsV4(page),
    };

    return gate6CandidateV4Strategy().predict(input, {
      id: "s4r4-metamorphic",
      tier: "A",
      description: "S4R4 metamorphic",
...
describe("surface ownership classifier", () => {
