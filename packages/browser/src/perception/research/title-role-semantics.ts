...
export type Gate6SurfaceKindV5 =
  "primary" | "blocking_dialog" | "alert" | "supplementary";

export interface Gate6SemanticSurfaceV5 {
  id: string;
  kind: Gate6SurfaceKindV5;
  blocking: boolean;
...
export interface Gate6SurfaceFactsV5 {
  available: boolean;
  ...
  surfaces: Gate6SemanticSurfaceV5[];
...
export async function collectGate6SurfaceFactsV5(
  page: Page,
): Promise<Gate6SurfaceFactsV5> {
  return page.evaluate<Gate6SurfaceFactsV5>(`(() => {
    ...
    const titleMetaCue = (value) => {
      ...
      const explicitDocumentWord = words.some((word) =>
        [
          "documentation",
          "docs",
          "tutorial",
      ...
      const guideAsDocumentType =
        text === "guide" ||
        text.endsWith(" guide") ||
        text.startsWith("guide to ") ||
        text.startsWith("guide for ");

      return explicitDocumentWord || guideAsDocumentType;
    };
