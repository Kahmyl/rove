...
async function classify(body: string, title = "Test", httpStatus?: number) {
  ...
  try {
    ...
    const input: Gate6CandidateV6Input = {
      signals: await pageSignals(page, httpStatus),
      evidence: evidence.evidence,
      surfaceFacts: await collectGate6SurfaceFactsV6(page),
    };

    return gate6CandidateV6Strategy().predict(input, {
      id: "s4r6-metamorphic",
      tier: "A",
      description: "S4R6 metamorphic",
...
describe("alert precedence classifier", () => {
