...
const artifactPath = join(
  root,
  "artifacts/agent-execution-drive-live-retry2-20260909-0430.json",
);
const nativeScreenshotPath = join(
  root,
  "artifacts/verification/browser-acceptance/drive-retry2-20260909-0430-native.png",
);
...
const driveFolderName =
  ...
  `Rove Browser Acceptance ${new Date()
    .toISOString()
    .replaceAll(/[-:TZ]/g, "")
    .slice(0, 12)
    .replace(/(\d{8})(\d{4})/, "$1-$2")}`;
...
const outcome = `Create one fresh Google Drive folder named ${driveFolderName}. Upload only the pre-launch attached rove-live-acceptance.txt, verify it, rename it to rove-live-acceptance-renamed.txt, create Archive, move the renamed file into Archive, and verify the exact Archive breadcrumb and single final file. Do not share, invite, email, delete, or alter unrelated Drive content.

Capture a Rove viewport screenshot showing the folder and file. From the exact folder, call Rove browser.navigate with https://drive.google.com/drive/my-drive so it creates a distinct history entry. Then call browser.back and require it to return to the exact folder URL before verifying the breadcrumb and file again. If a safely completed read-only history operation lands elsewhere, fresh-inspect it and use at most one alternate read-only Rove navigate route for this step.

Download rove-live-acceptance-renamed.txt exactly once through the browser. Use that name only to identify the remote source file. Omit the optional download_completed filename because this journey is discovering and reporting the actual collision-safe saved filename. Obtain the actual local saved filename from durable evidence, along with its evidence identifier, size, and available MIME information. Never retry because of a collision suffix or an uncertain dispatch. Report all six external mutation outcomes and receipts, the final verification observation, URL, page identity, browser observations, download observation/evidence identifiers, screenshot evidence ID, and exact visible description.

If the pre-launch attachment is unavailable, stop before creating the folder. Read-only stale/page-change before dispatch may be retried once via fresh inspect; an uncertain consequential action or download dispatch must never be retried without durable not-applied evidence.`;
...
function projectEvidence(task) {
  ...
  return {
    ...
    downloadFilenameMentions: (
      combined.match(/rove-live-regression-gate-20260907-b\.txt/gi) ?? []
    ).length,
    downloadSizes: [
