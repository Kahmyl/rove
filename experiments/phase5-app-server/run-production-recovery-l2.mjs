...
const l2Root = join(root, "experiments/phase5-app-server/l2");
const driverPath = join(l2Root, "desktop-host-driver.mjs");
const artifactRoot = join(
  root,
  "docs/experiments/artifacts/p5.9-production-recovery-l2",
);
const rendererRoot = join(root, "apps/companion/dist/renderer");
const temporaryRoot = await mkdtemp(join(tmpdir(), "rove-p59-l2-"));
...
function progress(message) {
  process.stdout.write(`[p5.9-l2] ${message}\n`);
}
...
const evidence = {
  ...
  scenarioManifest: "experiments/phase5-app-server/l2/scenarios.json",
  scenarios: results,
