...
function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      const nextNotice = await window.rove.getNotice();

      setNotice(nextNotice);

      const next = await window.rove.getSnapshot();
  ...
  const run = async (operation: () => Promise<CompanionSnapshot | null>) => {
    setBusy(true);
  ...
  const view = toCompanionViewModel(snapshot, notice);

  const performPrimaryAction = (action: CompanionPrimaryAction) => {
    if (action === "take_control") {
      void run(window.rove.takeControl);
      return;
    }

    if (action === "return_control") {
      void run(window.rove.returnControl);
      return;
    }

    if (action === "finish_capture") {
      void run(window.rove.finishSession);
    }
  };

  const connectionState = error === null ? "Connected" : "Offline";

  const quietAction = view.experience === "agent_working";

  const canEndFromDetails =
    view.hasSession && view.canFinish && view.experience !== "capture";

  return (
    <div className={["app", `experience-${view.experience}`].join(" ")}>
      <header className="topbar">
        ...
        <div
          className={[
            "connection",
            error === null ? "connection-online" : "connection-offline",
          ].join(" ")}
      ...
      <main className="content">
        <section className="task-surface">
          <div className="state-indicator" aria-hidden="true">
            <StateGlyph experience={view.experience} />
          </div>

          <p className="kicker">{loading ? "Connecting" : view.kicker}</p>

          <h1>{loading ? "Checking for a session…" : view.title}</h1>

          {!loading && (
            <>
              <p className="description">{view.description}</p>

              {view.supportingText !== undefined && (
                <p className="supporting">{view.supportingText}</p>
              )}
            </>
          )}
          ...
          {!loading &&
            ...
            view.primaryActionLabel !== undefined && (
              <button
                className={[
                  "primary-action",
                  quietAction ? "primary-action-quiet" : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
                disabled={busy || error !== null}
                onClick={() => performPrimaryAction(view.primaryAction)}
              >
          ...
          <section className="details">
            ...
            >
              ...
              <svg
                aria-hidden="true"
                viewBox="0 0 20 20"
                className={detailsOpen ? "chevron chevron-open" : "chevron"}
              >
                <path
            ...
            <div
              className={[
                "details-collapse",
                detailsOpen ? "details-collapse-open" : "",
              ].join(" ")}
            >
              <div className="details-inner">
                <div className="metrics">
                  <div className="metric">
                    <strong>{view.observationCount}</strong>
                ...
                <dl className="session-details">
                  <div>
                    <dt>Session</dt>
                    <dd className="session-id" title={view.sessionId}>
                      {view.sessionId}
                    </dd>
                  </div>
                ...
                {canEndFromDetails && (
                  <button
                    ...
                    type="button"
                    disabled={busy}
                    onClick={() => void run(window.rove.finishSession)}
                  >
                    End session
                  </button>
                )}
...
function StateGlyph({ experience }: { experience: CompanionExperience }) {
  if (experience === "handoff_waiting") {
    return (
      <svg viewBox="0 0 24 24" className="state-glyph">
        <path
  ...
  if (experience === "session_ended" || experience === "interrupted") {
    return (
      <svg viewBox="0 0 24 24" className="state-glyph">
        <path
  ...
  return (
    <svg viewBox="0 0 24 24" className="state-glyph">
      ...
      <circle cx="11.5" cy="8" r="1.7" fill="currentColor" />
      <path
...
createRoot(document.getElementById("root")!).render(
  <StrictMode>
    {surfaceMode === "follower" ? <FollowerApp /> : <App />}
  </StrictMode>,
);
