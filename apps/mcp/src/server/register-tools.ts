...
export interface ToolDefinition {
  ...
  present?(result: unknown): ToolResult;
}

export function toolDefinitions(runtime: RuntimeClient): ToolDefinition[] {
  return [
...
export function registerTools(server: Server, runtime: RuntimeClient): void {
  const tools = toolDefinitions(runtime);
  ...
  server.setRequestHandler(
    CallToolRequestSchema,
    async (request, extra): Promise<ToolResult> => {
      const tool = definitions.get(request.params.name);
