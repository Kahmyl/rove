...
describe("runtime HTTP API", () => {
  ...
  it("serves the complete authenticated browser, observation, evidence, and session path", async () => {
    ...
    expect(
      (await json(baseUrl, `/sessions/${sessionId}`, {}, authorization)).body,
    ...
    const typed = await json(
      baseUrl,
      `/sessions/${sessionId}/browser/type`,
      {
        method: "POST",
    ...
    const inspectedAgain = await json(
      baseUrl,
      `/sessions/${sessionId}/browser/inspect`,
      { method: "POST", body: "{}" },
      authorization,
    ...
    const clicked = await json(
      baseUrl,
      `/sessions/${sessionId}/browser/click`,
      {
        method: "POST",
        body: JSON.stringify({
          target: {
            pageId: nextInspection.pageId,
            revision: nextInspection.revision,
            ref: nextChange.ref,
          },
        }),
      },
    ...
    expect(stale.body).toMatchObject({
      ok: false,
      error: { code: "TARGET_STALE", retryable: true },
    });

    const pages = await json(
      baseUrl,
      `/sessions/${sessionId}/browser/pages`,
      {},
      authorization,
    );
    expect(pages.body).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: "page_01" })]),
    );
    const screenshot = await json(
      baseUrl,
      `/sessions/${sessionId}/browser/screenshot`,
      { method: "POST", body: "{}" },
      authorization,
    );
    ...
    expect(
      (
        await json(
          baseUrl,
          `/sessions/${sessionId}/evidence/${String(record.body.id)}`,
          {},
          authorization,
        )
      ).body,
    ).toMatchObject({ id: record.body.id });
    const observations = await json(
      baseUrl,
      `/sessions/${sessionId}/observations?afterSeq=0&limit=100`,
      {},
      authorization,
    );
    expect(
      (observations.body.items as { type: string }[]).map((item) => item.type),
    ).toEqual(
  ...
  it("serves authenticated explicit control operations and cancels disconnected waits", async () => {
    ...
    expect(requested.body).toMatchObject({
      ...
      handoff: { reason: "Manual update" },
    });
    const waitPromise = json(
      baseUrl,
      `/sessions/${sessionId}/control/wait?afterSeq=${String(requested.body.observationSeq)}&timeoutMs=1000`,
      {},
      authorization,
    );
    ...
    expect((await waitPromise).body).toMatchObject({
    ...
    const disconnected = fetch(
      `${baseUrl}/sessions/${sessionId}/control/wait?afterSeq=${String(resumed.body.observationSeq)}&timeoutMs=10000`,
      {
        headers: { authorization },
        signal: abort.signal,
      },
    );
    while (waits.waiterCount(sessionId) === 0)
    ...
    expect(
      (await json(baseUrl, `/sessions/${sessionId}/control`, {}, authorization))
        .body,
    ).toMatchObject({ status: "active", controller: "agent" });
    await json(
      baseUrl,
      `/sessions/${sessionId}/end`,
      { method: "POST" },
...
describe("Companion session discovery", () => {
  it("lists active sessions and filters Companion Mode", async () => {
    ...
    expect(active.body as unknown as unknown[]).toEqual([
    ...
    expect(afterEnd.body as unknown as unknown[]).toEqual([]);
