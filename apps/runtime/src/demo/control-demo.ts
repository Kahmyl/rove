...
import type { ControlMutationAuthority, ControlStatus, ControlWaitResult, PageInspection, Session } from "@rove/protocol";
...
try {
  ...
  const taken = await request<ControlStatus>(`/sessions/${agent.id}/control/take`, { method: "POST", body: JSON.stringify(controlAuthority(requested)) });
  ...
  const returned = await request<ControlStatus>(`/sessions/${agent.id}/control/return`, { method: "POST", body: JSON.stringify(controlAuthority(taken)) });
  ...
  const companionControl = await request<ControlStatus>(`/sessions/${companion.id}/control`);
  const voluntary = await request<ControlStatus>(`/sessions/${companion.id}/control/take`, { method: "POST", body: JSON.stringify(controlAuthority(companionControl)) });
  ...
  console.log(`companion return: ${(await request<ControlStatus>(`/sessions/${companion.id}/control/return`, { method: "POST", body: JSON.stringify(controlAuthority(voluntary)) })).controller}`);
