...
export class ControlService {
  ...
  assertCanRequestHuman(session: Session): void {
    ...
    throw new RoveError({
      code: "CONTROL_NOT_OWNED",
      message: "The agent does not own browser control.",
    });
  }
  ...
  assertCanTakeHuman(session: Session): void {
    ...
    if (
      ...
      throw new RoveError({
        ...
        message: "Agent Mode requires an explicit human handoff request.",
      });
    }
    throw new RoveError({
      code: "CONTROL_NOT_OWNED",
      message: "Human control cannot be taken from the current state.",
    });
  }
  ...
  assertCanReturnAgent(session: Session): void {
    ...
    if (session.mode === "capture") {
      throw new RoveError({
        code: "CONTROL_NOT_OWNED",
        message: "Capture Mode remains human-controlled.",
      });
    }
    if (
      session.status === "active" &&
      ...
      return;
    if (session.status === "paused" && session.controller === null) return;
    if (session.status === "active" && session.controller === "human") return;
    throw new RoveError({
      code: "CONTROL_NOT_OWNED",
      message: "Human control is not currently owned.",
    });
  }

  assertCanPauseAgent(session: Session): void {
    this.assertNonTerminal(session);
    if (
    ...
    throw new RoveError({
      code: "CONTROL_NOT_OWNED",
      message: "Only active agent control can be paused.",
    });
  }
