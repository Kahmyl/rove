...
class MemoryObservations implements ObservationStore {
  ...
  async append(_sessionId: string, observation: Observation) {
    this.items.push(observation);
  }
  async list(_sessionId: string, query: ObservationQuery = {}) {
    return this.items
      .filter((item) => item.seq > (query.afterSeq ?? 0))
...
async function setup() {
  ...
  return {
    ...
    sessions,
    observations,
    waits: new ControlWaitService(sessions, observations),
  };
}
...
describe("ControlWaitService", () => {
  it("returns the earliest already-persisted relevant event", async () => {
    ...
    await observations.append("ses_wait", {
      actor: "agent",
      type: "agent_clicked",
      data: {},
    });
    const requested = await observations.append("ses_wait", {
      actor: "agent",
    ...
    await expect(waits.wait("ses_wait", 0, 50)).resolves.toMatchObject({
      event: "human_requested",
      observationSeq: requested.seq,
    });
  });
  ...
  it("wakes for a future event and returns timeout as a normal result", async () => {
    ...
    const event = await observations.append("ses_wait", {
      actor: "human",
      type: "human_took_control",
    ...
    await expect(waits.wait("ses_wait", event.seq, 20)).resolves.toMatchObject({
      event: "timeout",
      status: "active",
    });
  });
  ...
  it("does not wake for a published event at or before afterSeq", async () => {
    const { observations, waits } = await setup();
    let settled = false;
    const pending = waits.wait("ses_wait", 5, 1_000).finally(() => {
      settled = true;
    });
    const older = await observations.append("ses_wait", {
      actor: "agent",
      type: "human_requested",
      data: { reason: "Older" },
    });
    ...
    const newer = await observations.append("ses_wait", {
      actor: "human",
      type: "human_took_control",
      data: { requested: true },
    });
  ...
  it("does not lose an event between initial query and waiter registration", async () => {
    ...
    await expect(waits.wait("ses_wait", 0, 1_000)).resolves.toMatchObject({
      event: "human_requested",
      handoff: { reason: "Race" },
    });
  ...
  it("unregisters a cancelled waiter without changing session state", async () => {
    ...
    expect(waits.waiterCount("ses_wait")).toBe(0);
    expect(await sessions.get("ses_wait")).toMatchObject({
      status: "active",
      controller: "agent",
    });
  });
